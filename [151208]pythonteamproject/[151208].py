...
def show_mypage():
    ...
    for row in result: 
        print(row)
        #if (row[2] == '인디밴드') or (row[2] =='인디(보컬솔로)'):
        #    recom[0] += 1
        #elif (row[2] == '통기타라이브') or (row[2] =='라틴팝/안데스음악') or (row[2] =='재즈밴드'):
        #    recom[1] += 1
        #elif (row[2] == '힙합') or (row[2] =='K-POP'):
        #    recom[2] += 1
        #elif (row[2] == '7080') or(row[2] == '댄스퍼포먼스(택견)') or (row[2] =='MC레크레이션') \
        #    or (row[2] =='MC,레크레이션') or (row[2] =='댄스퍼포먼스(태껸)') or (row[2] =='태껸'):
        #    recom[3] += 1
        #elif (row[2] == '아카펠라') or (row[2] =='합창'):
        #    recom[4] += 1
        #elif (row[2] == '피아노연주') or (row[2] =='오카리나연주') or (row[2] =='색소폰앙상블') or (row[2] =='색소폰연주') \
        #    or (row[2] =='우쿠렐레연주') or (row[2] =='피아노 7중주'):
        #    recom[5] += 1
        #elif (row[2] == '클래식(팬플룻/팝페라)') or (row[2] =='클래식(비올라)') or (row[2] =='클래식(현악5중주)') \
        #    or (row[2] =='클래식(팝페라)') or (row[2] =='클래식(금관앙상블)') or (row[2] =='클래식(트롬본앙상블)') \
        #    or (row[2] =='클래식(팬플릇팝페라)') or (row[2] =='클래식(피아노7중주)'):
        #    recom[6] += 1
    ...
    if index == 0:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%인디%' and f.pname=p.pname order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
    elif index == 1:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%통기타%' or pgenre like '%라틴%' or pgenre like '%재즈%' order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
    elif index == 2:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%힙합%' or pgenre like '%K-POP%' order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
    elif index == 3:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%7080%' or pgenre like '%택견%' or pgenre like '%태껸%' or pgenre like '%MC%' order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
    elif index == 4:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%아카펠라%' or pgenre like '%합창%' order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
    elif index == 5:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%연주%' or pgenre like '%색소폰%' or pgenre like '%피아노%' order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
    elif index == 6:
        cur = g.db.execute("select f.fpic, p.pname, p.pgenre, p.pplace, p.psdate, p.pedate, p.pnum from performance p, finfo f where p.pgenre like '%클래식%' order by random() limit 5;")
        result2 = [dict(pic=row[0], name=row[1], genre=row[2], place=row[3], time=row[4]+" ~ "+row[5], num=row[6]) for row in cur.fetchall()]
...
def linemap_result():
    ...
    cur = g.db.execute('select pnum, email from like')
    result2 = [dict(pnum=row[0], email=row[1]) for row in cur.fetchall()]
